    ...
    async (link: LinkModel) => {
      try {
        ...
        const res = await fetch(
          `/api/chatbots/${chatbot}/links/${link.id}/retrain`,
          {
            method: "POST",
          },
        );
  ...
  const deleteLink = useCallback(
    async (link: LinkModel) => {
      try {
        ...
        const res = await fetch(`/api/chatbots/${chatbot}/links/${link.id}`, {
          method: "DELETE",
        });
        const data: ApiResponse<LinkModel> = await res.json();
        if (!data.success) {
        ...
        queryClient.invalidateQueries(["links", chatbot]);
        toast({
